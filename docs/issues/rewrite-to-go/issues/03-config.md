...
### 2026-09-11 暂定（未经明确裁定）

用户指示「继续」推进，以下按推荐执行，**均标记为暂定，用户随时可否决**：

| 题 | 暂定结论 | 若否决需改什么 |
| --- | --- | --- |
| D1 | schema **以附录 A 为底**定稿（`.conf` 文件、删 `paths.exe` 段、keywords 路径扁平化） | 调整字段 |
| D4 | `secret_dir` 字面量 `none`／`None`／`NONE` **一律视为未配置** | 取消宽容，`none` 当目录名 |
| D6 | **超时值与开关类给默认值**（沿用旧配置里的 10 / 60 / 300 / true）；**账号类（`port` / `user` / `secret_dir` / `password`）保持必填** | 恢复"全必填" |

> D6 属**轻度行为变更**：旧版配置缺任一字段即报错，暂定后缺超时/开关字段将走默认值。账号类之所以坚持必填，是因为"用默认值连错机器"比"直接报错"更糟。

...
- **2026-09-14 更正（审计前自查）**：上面「`-m sudo` → `id -u` 返回 0」**不足以证明提权生效**——测试节点 172.28.118.49 本身是 WSL2 Ubuntu，登录用户就是 `root`（`id -u` 恒为 0），且 `sudo` 由 root 执行必然成功。**「非 root 用户 + sudo 提权」这条路径至今未验证**，需换非 root 登录账号重验。

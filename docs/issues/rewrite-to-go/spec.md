...
| D2 | **不保留事件流模型** | 旧：`init` / `progress` / `result` / `done` 四类 SSE 消息。新：**进度聚合**归 `internal/batch`、**结果统计**归 `internal/result` |
...
旧实现里有一批做法是**被 Python 或双进程逼出来的**，换成 Go 后可以走更直接的路。以下均为**实现层**决策，不改变用户可见行为。

| # | 旧实现途径 | Go 的做法 | 收益 |
| --- | --- | --- | --- |
| 1 | 命令 / 脚本经 **base64 通道**下发：本地编码 → 拼进命令行 → 远端 `base64 -d` → 管道给解释器 | `session.Stdin = strings.NewReader(内容)`，命令仅剩 `bash -lc '…; sudo bash'` | 去掉编解码；**消除命令行长度上限风险**（base64 让命令行膨胀 33%，大脚本会撞 `ARG_MAX`）；引号转义问题从根上消失；日志可直接打印原始内容 |
| 2 | 手写终端宽度与对齐：逐字符查 East Asian Width 的 `display_width` + `pad_to_width` / `pad` / `center`，散布在三处 | `lipgloss.Width()` / `.Width(n)` / `.Align()` | 一批手写函数消失。旧版曾因此出过"全角标点导致表格错位" |
| 3 | `-k` 三态靠**魔法字符串**：`nargs='?'` + `const='no_value'`，代码里判 `args.k == "no_value"` | pflag 的 `NoOptDefVal` | 不再需要魔法字符串 |
| 4 | 配置里三个凭据字段各写一段几乎相同的路径拼接 | 一段遍历 | 三段重复变一段 |
| 5 | 未知配置字段靠 pydantic `extra="forbid"` | 解码后取未识别键集合 | 报错能列出**具体是哪个键** |
| 6 | `password_security` 声明为 `str` 再 `str()` 兜底（YAML 弱类型），兼容 `3` 与 `"3"` | TOML 强类型，直接 `int` | 少一层转换与一类错误 |
| 7 | Windows 主密钥读取：`subprocess` 起 `reg query`，再正则解析 stdout | 直接读注册表 | 不起子进程、不解析文本 |
| 8 | 凭据错误码是字符串字面量，拼错会静默落入兜底分支 | 自定义字符串类型 + 常量 | 拼错编译不过 |
| 9 | 危险检测的"间接执行递归提取"手写实现（`bash -c` / `eval` / `ssh`） | 遍历语法树里的所有调用表达式 | 递归逻辑不用自己写；对管道 / `&&` / `$()` / 反引号覆盖更全 |

**明确不动（避免为整齐而整齐）：**

- **字段补全的三套函数**（端口 / 用户名 / 密码）不合并。三者看似同构，但优先级链、交互文案、记忆变量、询问时机各不相同；统一会变成一个多分支函数，比三个直白函数更难读（D32）。
...
| # | 决策 | 与旧实现的差异 |
| --- | --- | --- |
| D33 | 脚本检测：**先整体解析成语法树**；解析失败（脚本有语法错）再**回退逐行** | 旧：一律逐行解析——续行符、heredoc 会把一条命令割裂 |
| D34 | 多处命中时**列出全部命中**，按风险降序 | 旧：只报最高风险的一条，用户对次高危险一无所知却一并执行了 |
| D35 | 非交互模式下非 `forbidden` 的危险命令**放行但写入执行日志** | 旧：既不提示也不确认、**且不留任何痕迹**，事后无法回溯 |
...
本工具的形态是**一条跑一次就退的流水线**，没有长生命周期、没有需封装的并发共享状态。因此：

**主干与各功能环节一律以「函数 + 数据」表达。** 不引入无状态的"服务对象"，不按 Service / Manager / Handler 分层，不用类型层次表达"是一种"（Go 只能组合）。

**只有当满足下列任一条时，才引入带方法的类型：**
...
- 为**只有一个实现**的东西预先抽 interface（Go 谚语：*don't design with interfaces, discover them*）
- 用 struct 包住一组**无状态函数**，只为"看起来面向对象"
...
- 新目录只在**确实存在一个独立功能边界**时开，不为收纳零散代码而开
- `internal/common` 有明确准入标准（见第六节），它**不是**"放不进去就扔这儿"的地方

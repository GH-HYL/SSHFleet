...
| D27 | 忽略规则 | 工作区根 `.gitignore` 采用**白名单**写法（默认全忽略，只列出 `docs/`、`modules/`、`CONTEXT.md`、`AGENTS.md`、`README.md`、`CHANGELOG.md`）。个人文件（`个人开发规范.md`）、`tools/`、`test/` 刻意不纳入——本仓库将对外推送，只保留「代码 + 决策文档」 |
| D28 | 配置文件位置 | 查找基准为**当前工作目录**，路径 `./config/`。文件为 TOML 格式、**后缀 `.conf`** → `./config/SSHFleet.conf`。工程内 `config/` 与 `main.go` 同级，作为模板配置。部署形态：单文件可执行放 `/usr/bin/`，配置随工作目录走 |
...
- 工程根另置 `config/` 目录（与 `main.go` 同级），存放模板配置 `SSHFleet.conf`（见 D28）
...
**文件位置**：`./config/SSHFleet.conf`（TOML 格式，`.conf` 后缀，基准为当前工作目录，见 D28）。

原则：**结构简单清晰**。旧「程序路径 `paths.exe`」段随双进程架构消亡。日志字段数量待 `issues/01-structure.md` 的 S4（一个日志还是两个）裁定。

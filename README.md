...
```
> 白话解释：每次只能选一种「模式」（`-c`/`-s`/`-u`/`-d`/`-z` 五选一，`|` 表示「或」）；选了 `-c`/`-s`/`-u`/`-d` 时必须再带 `-f`（节点清单），上传/下载还要带 `-p`；其余都是可加可不加的选项。
...
| 参数          | 说明                                                        |
| ------------- | ----------------------------------------------------------- |
| `-f csv_file` | 节点信息的 CSV 文件或内联CSV文本（-c / -s / -u / -d 时必填） |
| `-p path`     | 远程目标节点路径 (-u / -d 时必填)                           |
...
| 参数               | 说明                                                                 |
| ------------------ | -------------------------------------------------------------------- |
| `-m mode`          | 执行权限: direct=用户权限, sudo=root权限                           |
| `-t timeout`       | 执行或传输超时 (秒)                                                 |
| `-T timeout`       | 连接超时 (秒)                                                       |
| `-n number`        | 并发执行或传输的数值                                                |
| `-r remark`        | 备注信息, 用于生成历史记录文件名后缀，默认自动生成                  |
| `--nobash`         | 命令模式: 不使用 bash 环境, 直接执行原始命令                        |
| `--disinteractive` | 跳过交互确认, 直接执行                                              |
...
> ⚠️ 小白最容易踩的坑：**密码和私钥口令都不要直接写进 CSV**，而是写一个"文件的路径"，那个文件里才是真正的内容（且密码/口令要做 Base64 编码）。这样能避免敏感信息以明文暴露在清单里。
...
**场景 A：所有服务器端口 / 用户名 / 密码都一样（最常见）**
只要按第 1 步准备好一个密码文件，CSV 可以极简——每行只写 IP，其余全留空（自动用配置默认值）：
```csv
192.168.1.10
192.168.1.11
192.168.1.12
...
**场景 C：用密钥登录（不用密码）**
第 4 列留空，第 5 列写你的 PEM 私钥文件路径（如 `~/.ssh/id_ed25519`）：
```csv
192.168.1.10,22,root,,~/.ssh/id_ed25519
```
私钥文件就是 SSH 标准的 `-----BEGIN ... PRIVATE KEY-----` 文本，**原样放进文件即可，不需要 Base64**。

**场景 D：密钥本身加密了（有 passphrase）**
再加第 6 列，指向"口令文件"（内容是该口令的 Base64，生成方式与第 1 步相同）：
```csv
...
- `./pw.txt` — 相对路径，会拼接配置里的 `account.password_dir`

#### 某一列留空会怎样（找不到值时的顺序）
1. 先看 CSV 这一列填了没；
2. 没填 → 用配置文件 `src/config/SSHFleet.yaml` 里的默认值（端口 / 用户名 / 密码 / 口令都有默认项）；
...
account:
  ...
  user: root                    # 默认登录用户名
  password_dir: ~/.MyPW         # 密码/口令文件所在目录（CSV 里写相对路径时会拼到这里）
...
| 配置段      | 关键参数                  | 小白解读                                                           |
| ----------- | ------------------------- | ------------------------------------------------------------------ |
| `account`   | port, user, password_dir, password | CSV 里没填端口/用户名/密码时用的默认值；`password` 是「密码文件路径」，文件内容才是真正的密码 |
| `account`   | key_passphrase            | 默认私钥口令文件（仅当用「加密过的密钥」登录才需要）；内容是该口令的 Base64，可被 CSV 第 6 列按节点覆盖 |
| `execution` | mode, timeout_*           | 执行权限（direct/sudo）、各种超时时间                               |
| `enable`    | output_to_xlsx, results_to_xlsx | 是否把结果导出成 Excel                                            |
...
A: 在第 5 列填你的 PEM 私钥文件路径（如 `~/.ssh/id_ed25519`），第 4 列留空即可，详见「CSV 文件格式」场景 C。如果私钥本身加密了（有 passphrase），再把口令文件路径填到第 6 列，详见场景 D。

...
# 仅用密钥登录：第五列为密钥文件路径（留空密码列）
192.168.1.10,10022,root,,~/.ssh/id_ed25519

# 密钥带密码：第六列为私钥密码（passphrase）
192.168.1.10,10022,root,,~/.ssh/id_ed25519,my-key-pass
```
...
密钥登录说明：
- 第五列为密钥文件路径（PEM 私钥），与密码列二选一；两者都填时优先用密钥
- 第六列为私钥密码（passphrase），仅在使用加密私钥时填写
- 密钥文件内容为 PEM 文本，UTF-8 编码，程序会先 Base64 解码再解析
- 节点同时配置密钥和密码时，密钥解析失败会自动回退使用密码
...
| `account`   | key\_dir, key\_passphrase           | 密钥缺省值：默认密钥目录、默认私钥密码 |

...
CSV 就是一份"服务器清单"：**纯文本文件，每行一台服务器，列之间用英文逗号 `,` 分隔**。不需要表头（第一行如果不是 IP 会自动跳过），以 `#` 开头的行是注释、会被忽略。你可以用记事本 / VSCode 直接编辑。

固定 **6 列，按顺序排列**（靠后的列可以留空，留空时会自动往下找默认值）：
...
密码文件 = 一个普通文本文件，内容是你服务器密码的 **Base64 编码**（不是密码原文）。
...
| 配置段      | 关键参数                            | 说明                                   |
| ----------- | ----------------------------------- | -------------------------------------- |
| `account`   | port, user, password\_dir, password | CSV 缺省值；`password` 为"内容为 Base64 编码明文密码"的文件路径 |
| `account`   | key\_passphrase                     | 默认私钥口令文件：内容为 Base64 编码的私钥口令；全局生效，可被 CSV 第 6 列按节点覆盖 |

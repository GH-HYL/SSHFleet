...
def parse_args(config: SSHFleetConfig) -> argparse.Namespace:
    ...
    try:
        ...
        # parser.add_argument('-tc', metavar='transfer_command', help=f'     传输模式专用，传输完成后远程执行命令')
        ...
        # parser.add_argument('-e', metavar='env', default='', type=str, help='                           环境变量，默认为空，可在配置文件进行配置 （ -c 或 -s 专用，可选填）') 
    ...
    # # 自动去除环境变量中的'export '，规范args.e的值
    # if args.e and args.e.startswith("export "):
    #     args.e = args.e[len("export ") :]
    #     print("环境变量不需要输入'export ' , 已自动为您去除")
...
def results_statistics(
    ...
    # 分离成功分类IP
    success_ips = (
        category_ip_map.pop(success_category, [])
        if success_category in category_ip_map
        else []
    )
...
def zip_latest_history(args, config: SSHFleetConfig):
    ...
    if args.z:
        if len(sys.argv) > 2:
            print(
                f"{color.COLOR_RED}[ERROR]{color.COLOR_RESET}{color.COLOR_YELLOW} [function:check_arguments]{color.COLOR_RESET} -z 模式不能与其他参数一起使用",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not os.path.isdir(history_dir):
        print(
            f"{color.COLOR_RED}[ERROR]{color.COLOR_RESET}{color.COLOR_YELLOW} [function:package_latest_history]{color.COLOR_RESET} 历史记录目录 '{history_dir}' 不存在",
            file=sys.stderr,
        )
    ...
    if not log_dirs:
        print(
            f"{color.COLOR_RED}[ERROR]{color.COLOR_RESET}{color.COLOR_YELLOW} [function:package_latest_history]{color.COLOR_RESET} 历史记录目录 '{history_dir}' 中没有日志文件夹",
            file=sys.stderr,
        )
        print("提示: 请先至少一次执行任务以生成历史记录")
        sys.exit(1)
    ...
    if not success:
        print("未找到可用的打包工具 (zipfile 或 tarfile)", file=sys.stderr)
